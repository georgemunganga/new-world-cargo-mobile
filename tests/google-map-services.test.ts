...
describe("Google map services", () => {
  it("restricts local autocomplete to the Lusaka service area and resolves coordinates", async () => {
    ...
    const suggestions = await service.autocompleteGooglePlaces("local", "Manda", [], { city: "Lusaka", latitude: -15.4, longitude: 28.3 });
...
it("does not invent a road route when Google is unavailable", async () => {
  process.env.EXPO_PUBLIC_GOOGLE_MAPS_API_KEY = "test-key";
  vi.stubGlobal("fetch", vi.fn(async () => new Response("{}", { status: 403 })));
  const { computeGoogleRoadRoute } = await import("../lib/services/maps/google-routes-service");
  await expect(computeGoogleRoadRoute({ latitude: -15.4, longitude: 28.3 }, { latitude: -15.3, longitude: 28.4 })).rejects.toThrow("Road route unavailable");
});
