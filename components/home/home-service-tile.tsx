...
export function HomeServiceTile({ label, icon, accent = false, onPress }: { label: string; icon: AppIconName; accent?: boolean; onPress: () => void }) {
  return <TouchableOpacity accessibilityRole="button" accessibilityLabel={`Choose ${label}`} onPress={onPress} activeOpacity={0.76} style={[styles.tile, accent && styles.tileAccent]}><View style={[styles.iconWrap, accent && styles.iconWrapAccent]}><AppIcon name={icon} size={22} color={accent ? nwcColors.primaryInk : nwcColors.brandNavy} /></View><Text numberOfLines={2} style={[styles.label, accent && styles.labelAccent]}>{label}</Text></TouchableOpacity>;
}

const styles = StyleSheet.create({
  tile: { width: "48%", minHeight: 105, borderRadius: 22, padding: 12, justifyContent: "space-between", backgroundColor: "#F5F8F9", borderWidth: 1, borderColor: "#E4EAED" },
  ...
  labelAccent: { color: nwcColors.white },
});
