...
export function ProgressSteps({ activeStep, steps: progressSteps = steps }: { activeStep: string; steps?: BookingProgressStep[] }) {
  const activeIndex = Math.max(0, progressSteps.findIndex((step) => step.id === activeStep));
  return <View accessibilityRole="progressbar" accessibilityValue={{ min: 1, max: progressSteps.length, now: activeIndex + 1 }} style={styles.progress}><View style={styles.progressMeta}><Text style={styles.progressLabel}>{`Step ${activeIndex + 1} of ${progressSteps.length}`}</Text><Text style={styles.progressName}>{progressSteps[activeIndex].label}</Text></View><View style={styles.progressTrack}>{progressSteps.map((step, index) => <View key={step.id} style={[styles.progressSegment, index <= activeIndex && styles.progressSegmentActive]} />)}</View></View>;
}
