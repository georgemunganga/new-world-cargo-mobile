...
export default function AppStateGalleryScreen() {
  ...
  return <Screen><View style={styles.page}><View style={styles.header}><View><SectionHeader title="App states" /></View><IconButton label="Go back" icon="arrow-left" onPress={() => router.back()} /></View><ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}><View style={styles.banner}><AppIcon name="flask-outline" size={21} color={nwcColors.primaryInk} /><Text style={styles.bannerText}>These controls are only for local development. They let the team review startup and recovery experiences while the app remains connected to live services.</Text></View><TouchableOpacity accessibilityRole="button" accessibilityLabel="Open physical device review" accessibilityHint="Open the manual accessibility and device review checklist" onPress={() => router.push("/dev/device-review" as Href)} activeOpacity={0.76}><Card style={styles.reviewEntry}><View style={styles.reviewIcon}><AppIcon name="cellphone-check" size={21} color={nwcColors.primaryInk} /></View><View style={styles.stateCopy}><Text style={styles.stateTitle}>Physical device review</Text><Text style={styles.stateDetail}>Manual small-screen, accessibility, keyboard, map, and system-bar checklist.</Text></View><AppIcon name="chevron-right" size={21} color={nwcColors.muted} /></Card></TouchableOpacity>{scenarios.map((item) => <TouchableOpacity key={item} accessibilityRole="button" accessibilityLabel={`Preview ${startupScenarioLabel(item)}`} onPress={() => chooseScenario(item)} activeOpacity={0.76}><Card style={[styles.stateCard, scenario === item && styles.selectedCard]}><View style={styles.stateCopy}><Text style={styles.stateTitle}>{startupScenarioLabel(item)}</Text><Text style={styles.stateDetail}>{descriptionFor(item)}</Text></View>{scenario === item ? <StatusBadge label="Selected" tone="success" icon="check" /> : <AppIcon name="chevron-right" size={21} color={nwcColors.muted} />}</Card></TouchableOpacity>)}</ScrollView></View></Screen>;
}
...
const styles = StyleSheet.create({
  ...
  reviewEntry: { minHeight: 84, flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: "#EAF4F8", borderColor: "#CAE3EC" },
  reviewIcon: { height: 42, width: 42, borderRadius: 14, alignItems: "center", justifyContent: "center", backgroundColor: nwcColors.primary },
