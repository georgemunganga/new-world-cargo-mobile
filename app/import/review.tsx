...
export default function ImportReviewScreen() {
  ...
  return (
    <ShipmentReviewScreen
      ...
      detail="Review the USD price and import details before confirming."
      continueLabel={
        pricing.loading
          ? "Updating price..."
          : pricing.error
    ...
    >
      <ShipmentTicket
        draft={importDraft}
        onEditRoute={() => router.push("/import/route" as never)}
        onEditCargo={() => router.push("/import/cargo" as never)}
        onEditContacts={() => router.push("/import/consignee" as never)}
      />
      <SummaryRow
        label="Shipping method"
        value={importDraft.method === "air" ? "Air freight" : "Sea freight"}
        onEdit={() => setMethodEditOpen(true)}
      />
      {importDraft.supportingDocument ? (
        <SummaryRow
          label="Document"
          value={importDraft.supportingDocument.name}
          onEdit={() => router.push("/import/cargo" as never)}
        />
      ) : null}
      <ShipmentCharges
        quote={importDraft.quote}
      ...
      <BookingQuickEditDrawer<"air" | "sea">
        visible={methodEditOpen}
        overline="Import method"
        title="Change freight method"
        detail="Change this preference without leaving your review."
        value={(importDraft.method ?? "air") as "air" | "sea"}
        choices={importMethodEditChoices}
        approveLabel="Update method"
        onDismiss={() => setMethodEditOpen(false)}
        onApprove={(method) => {
          updateImportDraft({ method });
