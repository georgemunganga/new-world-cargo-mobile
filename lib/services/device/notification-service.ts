...
async function loadNotifications() {
  ...
  return require(moduleName) as typeof import("expo-notifications");
}
...
export const notificationService = {
  async registerForPush(): Promise<NativeServiceResult<PushRegistration>> {
    ...
    if (!Notifications) return nativeUnavailable("missing-native-module", "Push notifications require a development or production build.");
  ...
  async scheduleLocalNotification(input: LocalNotificationInput): Promise<NativeServiceResult<{ id: string }>> {
    ...
    const Notifications = await loadNotifications();
    if (!Notifications) return nativeUnavailable("missing-native-module", "Local notifications require a development or production build.");
